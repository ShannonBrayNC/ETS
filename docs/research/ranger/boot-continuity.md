...
| Deleted or substituted prior chain | Local database, export, or verifier input | A different history is presented as the predecessor | Current checkpoint must name the exact verified prior signed head | Independently retained head checkpoint | Current checkpoint, prior chain, and expected prior head | Delete a prior suffix or supply a different valid head |
...
| Replay of an otherwise valid pair or omitted historical boot | Verifier request or incomplete export | Old but valid lineage is presented as current | Not detected by pairwise verification | Externally retained latest sequence/head and freshness policy | Registry state or witnessed latest checkpoint | Replay the same valid pair against a future registry contract |
...
| Current-chain suffix deletion | Local storage or export after checkpoint creation | Recent events disappear without an internal gap | A future retained checkpoint would bind the current head | External checkpoint publication and expected-event policy | Later witnessed head or source completeness policy | Truncate current suffix before and after a later checkpoint exists |
