...
Parent: #541
Implementation issues: #579, #583

## Purpose

This contract governs the Microsoft P0 72-hour soak after a successful RC1D pre-soak reconciliation. It replaces the invalidated #479 hardcoded source/image binding and supersedes RC1D v1 freeze handoffs that did not independently prove post-recovery live Purview durable-state health. Historical and invalidated attempts are never resumed or counted.
...
6. After that recovery, the deployed Gateway executes the shipped Purview polling path and a later RC1C read-only preflight independently proves the live durable Purview runtime healthy: subscription enabled, checkpoint and last-success present, `purview_healthy_observation=true`, `purview_gap_open=false`, retry count zero, and no active lease.
7. The Gateway durable-state probe is healthy and reconciled.
8. The Gateway relay/gap recovery passes and leaves no failed relay state.
9. RC1D pre-soak reconciliation passes and emits `ets.live_microsoft.rc1d_pre_soak_candidate.v2` with `rc1c_live_health_verified=true`, `freeze_ready=true`, `candidate_frozen=false`, and `soak_clock_started=false`.

The post-recovery RC1C live-health preflight is query-only. It does not edit durable state or manufacture recovery. A candidate whose live Purview runtime is still gapped is not freeze-ready even when the protected RC1C subscription lifecycle and synthetic polling matrix passed.

Do not publish Q0 until all source changes required for the soak coordinator and live runtime have merged. A source change after Q0 or RC1D invalidates that exact-source handoff.

## Initial manual start

Dispatch `live-m365-source-to-proof-soak.yml` from the exact unchanged `main` SHA using:

- `rc1d_workflow_run_id`: the exact successful RC1D v2 reconciliation run;
...
The coordinator validates that the RC1D run is a completed successful `workflow_dispatch` run from `main`, at the coordinator SHA, and from `.github/workflows/live-microsoft-rc1d-pre-soak-reconciliation.yml`. It downloads only the exact retained RC1D artifact, requires schema `ets.live_microsoft.rc1d_pre_soak_candidate.v2` and `rc1c_live_health_verified=true`, and derives the frozen source, image, and digest from that artifact. Browser input or an arbitrary source/image string cannot establish the freeze. RC1D v1 artifacts are not accepted.
...
3. **RC1C Purview read-only** — `live-microsoft-rc1c-preflight.yml` re-proves the exact `ActivityFeed.Read` identity, `Audit.General` subscription visibility and enabled state, live checkpoint/last-success presence, `purview_healthy_observation=true`, `purview_gap_open=false`, zero retry state, inactive lease, and the Graph deferral boundary.
...
The soak coordinator never dispatches `live-microsoft-rc1c-subscription-recovery.yml`. Start/stop/restart subscription mutation and destructive/fault recovery belong before freeze only. The same live Purview health predicates used to make RC1D v2 freeze-ready are re-proved on every governed soak observation.
...
- RC1D v2 workflow run ID and evidence set;
...
The attempt invalidates on a failed coordinator run, frozen-source drift, monitoring-gap breach, child canary failure, candidate/image mismatch, Purview subscription not enabled, missing Purview checkpoint/last-success state, an unhealthy Purview observation, an open Purview collection gap, Purview retry state or active lease, failed Gateway relay state, Graph subscription/public-callback appearance, or any public evidence that retains customer identifiers or reusable credentials.

An invalidated attempt is not resumed. A replacement attempt requires an explicit manual dispatch with a fresh validated RC1D v2 handoff. `restart_active=true` explicitly invalidates an existing active attempt before starting the replacement.
...
When both the 72-hour duration and 72-successful-observation minimums are met without invalidation, the coordinator records the passed state and closes #541 as completed. The retained source/image pair, RC1D v2 live-health evidence, and hourly canary evidence become the Microsoft P0 soak qualification record.
