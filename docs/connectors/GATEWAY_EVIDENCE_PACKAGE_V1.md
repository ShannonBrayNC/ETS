...
system, source record, or transformation profile fails closed. The event-level source system and the
connector capture metadata's `connector_source_system` must both agree with package provenance; a
committed contradictory source declaration is not silently interpreted. If the event contains a
committed connector instance ID, the package must carry that same ID; it cannot silently omit known
instance provenance.
...
`affects_cryptographic_verification` to `false`. Gap IDs must be unique within one package; duplicate
instances of the same `gap_id` are rejected rather than leaving package consumers to choose between
potentially conflicting snapshots. A gap declaration is accepted only when its source system and
connector instance both match event-bound package provenance, its detection timestamp is not after
package export, and its latest `updated_at_utc` state was known by export time. The `updated_at_utc`
value must also be at least as late as every reconciliation-start, resolution, and acknowledgement
timestamp carried by the declaration, so it represents the latest included lifecycle state rather
than an earlier snapshot. A gap from another SharePoint connector instance therefore cannot be
attached merely because both instances share the same Microsoft source family.

## Export snapshot time

`exported_at_utc` describes the product-layer package snapshot. It must not precede the proof tree
head embedded in the package, and gap detection/update state carried by the package must be known by
that export time.

The embedded `EvidenceEvent.created_at_utc` is not used as the package commit clock. Connector capture
maps the source/capture observation time into that event field, and source clocks may legitimately be
ahead of Gateway wall-clock within their declared clock-quality semantics. A package may therefore be
exported before the event's source-observation timestamp while still carrying a valid ETS proof.
Rejecting that case would incorrectly convert source-clock skew into a package-integrity failure.

These checks constrain ETS package/proof chronology only. They do not alter proof validity, normalize
source time, or claim that ETS controls the source system's original event clock.
