...
This directory contains public-safe implementation guidance for ETS vertical integrations, developer onboarding, security hardening, certificate generation, public verification experiences, policy routing, threat modeling, AI governance, DevSecOps, and audit replay.
...
- [ETS Policy-Gated Routing Guide](ETS_POLICY_GATED_ROUTING_GUIDE.md) - decision model and Python examples for routing verified, missing, sensitive, external-release, civic, and invalid evidence into automation, human review, quarantine, rejection, archive, or restricted-release outcomes.
- [ETS Threat Model and Abuse-Case Guide](ETS_THREAT_MODEL_ABUSE_CASE_GUIDE.md) - threat model for false metadata, omitted events, tampered proofs, stale tree heads, tenant bleed, public manifest leaks, certificate overclaims, automation confusion, and supply-chain abuse.
- [ETS AI Governance and Agent Accountability Guide](ETS_AI_GOVERNANCE_AGENT_ACCOUNTABILITY_GUIDE.md) - implementation guide for AI prompts, retrieved context, tool calls, model outputs, human review, policy routes, and claim-safe AI certificates.
- [ETS DevSecOps and Software Supply Chain Guide](ETS_DEVSECOPS_SUPPLY_CHAIN_GUIDE.md) - implementation guide for commits, PRs, CI runs, test reports, SBOMs, artifact digests, scans, release approvals, deployments, rollbacks, and release gates.
- [ETS Audit Replay and Forensics Guide](ETS_AUDIT_REPLAY_FORENSICS_GUIDE.md) - replay guide for recomputing event hashes, verifying inclusion proofs, comparing tree heads, rebuilding certificates, rebuilding policy routes, gap reporting, and forensic handling.
