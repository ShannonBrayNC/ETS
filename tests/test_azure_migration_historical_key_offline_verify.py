...
def _set_expected_hashes(monkeypatch: pytest.MonkeyPatch, key_path: Path) -> None:
    ...
    monkeypatch.setattr(offline, "_EXPECTED_PEM_SHA256", hashlib.sha256(pem).hexdigest())
    monkeypatch.setattr(offline, "_EXPECTED_DER_SHA256", hashlib.sha256(der).hexdigest())
...
def test_verify_offline_rejects_wrong_key_id(
    ...
    _set_expected_hashes(monkeypatch, key_path)
    wrong = "https://source.example/keys/ets-tree-head/" + offline._SOURCE_KEY_VERSION
