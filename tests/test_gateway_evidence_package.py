...
def test_package_export_snapshot_rejects_future_proof_or_gap_state() -> None:
    package = _package()

    with pytest.raises(ValidationError, match="export precedes committed event"):
        GatewayEvidencePackageV1(
            proof_bundle=package.proof_bundle,
            source_provenance=package.source_provenance,
            gap_declarations=package.gap_declarations,
            exported_at_utc=NOW - timedelta(seconds=1),
        )

    future_tree_head = package.proof_bundle.tree_head.model_copy(
        update={"created_at_utc": NOW + timedelta(minutes=2)}
    )
    future_bundle = package.proof_bundle.model_copy(update={"tree_head": future_tree_head})
