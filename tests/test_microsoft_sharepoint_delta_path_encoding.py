...
def test_drive_delta_continuation_accepts_equivalent_percent_encoding() -> None:
    profile = sharepoint_drive_delta_request_profile(
        "tenant-prod",
        _tenant(),
        "b!qualified-drive",
    )
    ...
    for continuation in (
        "https://graph.microsoft.com/v1.0/drives/b!qualified-drive/"
        "root/delta(token='opaque')",
        "https://graph.microsoft.com/v1.0/drives/b!qualified-drive/"
        "delta(token='opaque')",
        "https://graph.microsoft.com/v1.0/drives/b%21qualified-drive/"
        "delta?token=opaque",
    ):
        assert validate_sharepoint_delta_url(profile, continuation) == continuation


def test_drive_delta_continuation_rejects_encoded_path_separator() -> None:
    profile = sharepoint_drive_delta_request_profile(
        "tenant-prod",
        _tenant(),
        "b!qualified-drive",
    ...
    continuation = (
        ...
        "b!qualified-drive%2Fother/delta(token='opaque')"
    )
...
def test_drive_delta_continuation_rejects_foreign_drive_after_decoding() -> None:
    profile = sharepoint_drive_delta_request_profile(
        "tenant-prod",
        _tenant(),
        "b!qualified-drive",
    ...
    continuation = (
        "https://graph.microsoft.com/v1.0/drives/"
        "b!other-drive/delta(token='opaque')"
    )
