...
PRINCIPAL = "spiffe://example.test/workload/okta-system-log"
...
class OnePageOktaClient:
    def __init__(self, page: OktaSystemLogPage) -> None:
        self.page = page
        self.closed = False
...
def _instance() -> ConnectorInstanceV1:
    return ConnectorInstanceV1(
        ...
        instance_id="okta-system-log-prod",
...
def _registration() -> SourceRegistration:
    return SourceRegistration(
        principal=PRINCIPAL,
        source_id="okta-system-log-authoritative",
        source_system="okta.system_log",
        tenant_id="tenant-authoritative",
        workspace_id="workspace-authoritative",
        adapter_id="okta.system_log",
        adapter_version="1.0",
        event_type="okta.system_log.observed",
        classification="internal",
        redaction_profile="okta-system-log-redaction-v1",
        minimization_profile="okta-system-log-metadata-v1",
        redacted_keys=frozenset({"secret"}),
        clock_quality="synchronized",
    )


def _adapter(page: OktaSystemLogPage) -> OktaSystemLogAdapter:
...
def _page(marker: str = "raw-source-marker") -> OktaSystemLogPage:
...
def test_okta_page_commits_before_checkpoint_is_released(tmp_path: Path) -> None:
    marker = "RAW-OKTA-GATEWAY-MARKER"
    ...
    assert result.checkpoint_to_persist.cursor == NEXT_URL
...
def test_okta_backpressure_withholds_checkpoint_and_append(tmp_path: Path) -> None:
...
def test_okta_partial_commit_withholds_checkpoint_then_recovers(tmp_path: Path) -> None:
