#!/usr/bin/env python3
"""Prepare destination Lantern custom domains and managed TLS without routing cutover."""
...
def _ensure_domain(resource_group: str, host: str, domain_name: str) -> dict[str, Any]:
    ...
    if payload is None:
        ...
        payload = _show_domain(resource_group, domain_name)
    if not isinstance(payload, dict):
        raise MigrationControlError(f"Gate 8B custom domain is unavailable: {host}")
    if str(payload.get("hostName", "")).casefold() != host.casefold():
...
def _domain_snapshot(host: str, domain_name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "validation_state": str(payload.get("domainValidationState", "")),
        ...
        "validation_token": str(validation.get("validationToken", "")),
    }


def _approved(item: dict[str, Any]) -> bool:
    return item["validation_state"] in {"Approved", "Valid"}


def _tls_ready(item: dict[str, Any]) -> bool:
    return (
        _approved(item)
...
def _ensure_validation_token(
    resource_group: str,
    host: str,
    domain_name: str,
    item: dict[str, Any],
) -> dict[str, Any]:
    if _approved(item) or item.get("validation_token"):
        return item
    _az_mutate(
        [
            "afd",
    ...
    for _attempt in range(24):
        ...
        if payload is not None:
            refreshed = _domain_snapshot(host, domain_name, payload)
            if refreshed.get("validation_token") or _approved(refreshed):
                return refreshed
        time.sleep(5)
    raise MigrationControlError(f"Gate 8B validation token is unavailable: {host}")
...
def _associate_approved_domains(
    ...
    target_ids = {str(item["id"]) for item in snapshots if item.get("id")}
    current_ids = {
        str(item.get("id", ""))
        for item in current
    ...
    approved_ids = sorted(
        str(item["id"]) for item in snapshots if _approved(item) and item.get("id")
    )
    if not approved_ids:
        return
...
def _wait_domain(resource_group: str, host: str, domain_name: str) -> dict[str, Any]:
    ...
    for _attempt in range(40):
        ...
        if not _approved(latest) or _tls_ready(latest):
            return latest
        time.sleep(15)
    if latest is None:
...
def prepare(
    *,
    ...
    for host, name in _DOMAINS.items():
        payload = _ensure_domain(resource_group, host, name)
        item = _domain_snapshot(host, name, payload)
        snapshots.append(_ensure_validation_token(resource_group, host, name, item))
    ...
    for item in final_domains:
        if _tls_ready(item):
            ...
            tls_hosts.append(str(item["host"]))
        else:
            token = str(item.get("validation_token", ""))
            if not token and not _approved(item):
                raise MigrationControlError(
                    f"Gate 8B DNS validation token missing for {item['host']}"
                )
            pending.append(
                {
                    "host": item["host"],
    ...
    domains = [
        {
            ...
            "status": "TLS_READY" if item["host"] in tls_hosts else "DNS_VALIDATION_REQUIRED",
        }
        for item in final_domains
    ]
    all_ready = len(tls_hosts) == len(_DOMAINS)
    return {
