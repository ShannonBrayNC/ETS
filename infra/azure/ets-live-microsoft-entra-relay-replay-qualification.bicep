...
RESULT_MARKER = "ETS_ENTRA_RELAY_REPLAY_B64="
...
def request_json(opener, token, method, path, body=None):
    headers = {
        ...
        "User-Agent": "ets-microsoft-entra-relay-replay-qualification/1.0",
    }
    data = None
    if body is not None:
...
try:
    ...
    for event_id, event_hash, body in qualified:
        proof_path = None
        replay_ok = False
        try:
...
qualification_pass = (
    ...
    and counts["terminal_entra_count"] == counts["terminal_total"]
    and counts["terminal_profile_count"] == counts["terminal_total"]
    ...
    and counts["proof_endpoint_ok"] == counts["replay_accepted"]
    and counts["proof_endpoint_failure"] == 0
)

result = {
    "schema_version": "ets.live_microsoft_entra.relay_replay_qualification.v1",
    **counts,
...
resource replayJob 'Microsoft.App/jobs@2025-01-01' = {
  ...
  properties: {
    ...
    template: {
      ...
      containers: [
        {
          name: 'microsoft-entra-relay-replay-qualification'
